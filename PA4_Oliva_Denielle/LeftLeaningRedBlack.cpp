...
LLTB_t* LeftLeaningRedBlack::findParent(VoidRef_t ref){
	LLTB_t *pNode = m_pRoot;
	...
	while (pNode != nullptr){
		...
		if(pNode->pLeft){
			if(pNode->pLeft->Ref.Key == key){
				return pNode;
			}
		}

		if(pNode->pRight){
			if(pNode->pRight->Ref.Key == key){
				return pNode;
			}
		}
...
bool LeftLeaningRedBlack::getColor(){
	return m_pRoot->IsRed;
}